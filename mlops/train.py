...
def train_loop_per_worker(config: dict) -> None:  # pragma: no cover, tested via train workload
    ...
    # Model
    model = models.FatigueNet(fc_size=fc_size)
...
def train_model(
    ...
    # Dataset
    ds = data.load_data(dataset_loc=dataset_loc, subset='train', participant=0, num_samples=train_loop_config["num_samples"])
    train_ds, val_ds = data.group_split(ds, test_size=0.2)
    targets = train_ds.unique(column="targets")
    train_loop_config["num_classes"] = len(targets)
